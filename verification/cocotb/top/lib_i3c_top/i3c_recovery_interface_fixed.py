...
class I3cRecoveryInterfaceFixed(I3cRecoveryInterface):
    ...
    async def command_write(self, address, command, data=None, force_pec_error=False,
        ...
        if start:
            ...
            # After T-bit abort: Sr already generated, bus normalized to
            # SCL=L. Skip send_start(), go directly to 0x7E/W header.
            await self.controller.write_addr_header(I3C_RSVD_BYTE)
            await self.controller.send_start()
            ack = await self.controller.write_addr_header(address)
    ...
    async def command_read_abort(self, address, command, abort_after_bytes=3, stop=True):
        """
        ...
        the last byte to read. This generates an Sr on the bus. If stop=True,
        a P follows (Sr+P). If stop=False, the bus remains active after the
        Sr for chaining into a subsequent transaction.
        ...
        Args:
            ...
            stop: If True (default), send P after the T-bit Sr and release bus.
                  If False, leave bus active after Sr for chaining.

        Returns:
            List of bytes read before the abort.
        """
        # Write phase: send command + PEC
        xfer = [command]
        pec = int(self.pec_calc.checksum(bytes([address << 1] + xfer)))
        xfer.append(pec)
        ...
        bytes_read = []
        if ack:
            # T-bit abort on the last byte generates Sr
            for i in range(abort_after_bytes):
                try:
                    is_abort_byte = (i == abort_after_bytes - 1)
                    byte, tgt_stop = await self.controller.recv_byte_t_bit(stop=is_abort_byte)
        ...
        if stop:
            # Sr already generated by T-bit, complete with P
            await self.controller.send_stop(pull_scl_low=False)
            self.controller.give_bus_control()
        else:
            await self.controller.normalize_after_tbit_abort()
