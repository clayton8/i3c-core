...
module i3c_target_fsm import i3c_pkg::*; (
  ...
  input  logic scl_negedge_i, // For assertions onlu
  ...
  input  i3c_byte_t tx_fifo_rdata_i,   // byte in tx_fifo to be sent to host
  ...
  assign ibi_status_o = ibi_status_d;
    ...
    case (state_q)
      ...
      IbiDriveAddr: begin
        ...
        if (bus_tx_rsp_i.done) begin
          ...
          ibi_status_we_o = 1'b1;
          ibi_status_d    = IbiFailureNack; // TODO extend the encoding
      ...
      IbiTbitCont: begin
        ...
        if (bus_tx_rsp_i.abort) begin
          ...
          ibi_byte_flush_o = 1'b1;
          ibi_status_we_o  = 1'b1;
          ibi_status_d     = IbiFailurePartialData;
          state_d = RxFByte;
  ...
  // TODO this is not true every time we wait for a start! Also, just make this a simple pulse!
